...
{
    // Clear previous rules so re-initialisation (theme change) does not
    // keep appending duplicate rules.
...
void Highlighter::syntaxColorUpdateHandler(HighlightColor &colors)
{
    ...
    // so repeated theme changes no longer duplicate rules.
    initColors();
    rehighlight();
}
